...
V2의 두 행은 정책에 일본 숙박 한도가 없어서 “제공된 규정으로는 해외 출장 한도를 확인할 수 없으니 재무팀에 확인하라”는 취지로 답했다. V1의 세 모델 D04도 같은 이유로 3점이었다. 이 행들은 모두 `not_covered`와 `SCOPE-2026`으로 업무 검사를 통과했다. 올바른 보류를 인정하는 업무 전용 평가 기준은 전문가와 설계해 새 실험으로 확인할 수 있다.
