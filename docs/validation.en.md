...
| Why did a specific answer fail? | `<label>/responses.jsonl` and `<label>/evaluation-results.json` | Match **the same `row_id` within one label**; inspect `business_grade → checks`, scores/reasons in evaluation `results`, and the source `trace_id` |
