...
| red team | 스캔은 의도적으로 유해한 프롬프트(폭력, 혐오·불공정)를 보낸다. 조직에서 허용하는지 확인하고, 허용되지 않으면 레벨 3의 3절은 건너뛴다. 조별 스캔은 `<LAB_PREFIX>-red-team-sol`이라는 평가로 실행된다. Sol 배포에 공격 6건을 약 1분 동안 보내며, New Foundry의 **Evaluations → Red team** 탭에 나온다. |
